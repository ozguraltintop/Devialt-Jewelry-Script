...
$klasord="../resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into bolge_yorum set adi=:adi,durum=:durum,mesaj=:mesaj");
	$ekle = $simdi->execute(array("adi"=>$adi,"durum"=>$durum,"mesaj"=>$mesaj));
	if($ekle){
		
		
		
		  $sonid=$db->query("select * from bolge_yorum order by id desc")->fetch(PDO::FETCH_ASSOC);
...
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong> Bölgesi  Yorum Başarıyla Eklendi!</strong>
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM bolge_yorum WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update bolge_yorum set adi=:adi,durum=:durum,mesaj=:mesaj where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"durum"=>$durum,"mesaj"=>$mesaj,"id"=>$id));
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong> Bölgesi Yorum Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from bolge_yorum where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title> Bölge Yorum Ekle - <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Bölge Yorum   Ekle</h5>
                                    ...
                                    <form method="post" enctype="multipart/form-data" >
                                        ...
                                        <label for="floatingInput">Hizmet Bölgesi Adı</label>
                                          ...
                                          <textarea class="form-control" name="mesaj" rows="10" cols="100"><?=$guncelle['mesaj']?></textarea>
