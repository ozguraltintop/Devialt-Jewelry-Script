...
imagedestroy($content);
	unlink ('../resimler/'.$resim1);
	 ...
	 $simdi = $db->prepare("insert into varyant_kategori set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from varyant_kategori order by id desc")->fetch(PDO::FETCH_ASSOC);
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmpd))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM varyant_kategori WHERE id = '$duzenlenecek_id'")->fetch(PDO::FETCH_ASSOC);
	...
	else
	{
		...
		if ($_FILES["resim"]["type"] =="image/gif" || $_FILES["resim"]["type"] =="image/png"|| $_FILES["resim"]["type"] =="image/jpg"|| $_FILES["resim"]["type"] =="image/jpeg") 
		{
		    	...
		    	$ayar_kaydi = $db->query("SELECT * FROM varyant_kategori WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
...
imagedestroy($content);
	 ...
	 $simdi1 = $db->prepare("update varyant_kategori set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from varyant_kategori where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Varyant Kategori Ekle - <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Varyant Kategori  Ekle</h5>
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Varyant Kategori Resmi</label>
