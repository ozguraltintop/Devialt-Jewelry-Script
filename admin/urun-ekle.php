...
imagedestroy($content);
	unlink ('../resimler/'.$resim1);
	 ...
	 $simdi = $db->prepare("insert into urunler set stok=:stok,urun_kodu=:urun_kodu,varyant=:varyant,fiyat=:fiyat,adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	$ekle = $simdi->execute(array("stok"=>$_POST['stok'],"urun_kodu"=>$_POST['urun_kodu'],"varyant"=>$_POST['varyant'],"fiyat"=>$fiyat,"adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"eklenme_tarihi"=>$tarih));
	if($ekle){
...
unlink ('../resimler/'.$resim1);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update urunler set stok=:stok,urun_kodu=:urun_kodu,varyant=:varyant,fiyat=:fiyat,adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("stok"=>$_POST['stok'],"urun_kodu"=>$_POST['urun_kodu'],"varyant"=>$_POST['varyant'],"fiyat"=>$fiyat,"adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Ürün sırası</label>
                                      </div>
                                        ...
                                        <label for="floatingInput">Ürün Adı</label>
                                          ...
                                          <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="fiyat" placeholder="Ürün Fiyatı" value="<?=$guncelle['fiyat']?>">
                                        <label for="floatingInput">Ürün Fiyatı</label>
                                      </div>
                                      
                                       <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="stok" placeholder="Ürün Fiyatı" value="<?=$guncelle['stok']?>">
                                        <label for="floatingInput">Ürün Stok (Adet)</label>
                                      </div>
                                      
                                      
                                       <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="urun_kodu" placeholder="Ürün Adı" value="<?=$guncelle['urun_kodu']?>">
                                        <label for="floatingInput">Ürün Kodu</label>
                                      </div>
                                        ...
                                        <?php
                                        $urun_kategori = $db->query("select * from urun_kategori where kategori='0' order by id desc",PDO::FETCH_ASSOC);
                               ...
                               <?php }}?>
                                        ...
                                        <div class="mb-3">
                                          ...
                                          <select class="form-select" name="varyant" >
                                        <option selected>Varyant Seçiniz</option>
                                        <?php
                                        $urun_kategori = $db->query("select * from varyant_kategori where kategori='0' order by id desc",PDO::FETCH_ASSOC);
										if($urun_kategori->rowCount()){foreach($urun_kategori as $urunkat){
										?>
                                        <option value="<?=$urunkat['id']?>" <?php if($urunkat['id']==$guncelle['varyant']){?> selected <?php }?>><?=$urunkat['adi']?></option>
                               <?php }}?>
                                      </select>
                                      </div>
