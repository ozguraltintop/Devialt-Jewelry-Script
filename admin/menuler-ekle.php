...
if($_POST['kaydet'] and $_GET['islem']==''){
	 ...
	 $simdi = $db->prepare("insert into menuler set adi=:adi,sira=:sira,kategori=:kategori,durum=:durum,linki=:linki");
	$ekle = $simdi->execute(array("adi"=>$adi,"sira"=>$sira,"kategori"=>$kategori,"durum"=>$durum,"linki"=>$_POST['linki']));
	if($ekle){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Menü Başarıyla Eklendi!</strong> 
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	 ...
	 $simdi1 = $db->prepare("update menuler set adi=:adi,sira=:sira,kategori=:kategori,durum=:durum,linki=:linki where id=:id");
	$ekle1 = $simdi1->execute(array("adi"=>$adi,"sira"=>$sira,"kategori"=>$kategori,"durum"=>$durum,"linki"=>$linki,"id"=>$id));
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Menü Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from menuler where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Menü Ekle - <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Menü  Ekle</h5>
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <label for="floatingInput">Menü Adı</label>
                                           ...
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="linki" placeholder="Kategori Adı" value="<?=$guncelle['linki']?>">
                                        <label for="floatingInput">Menü Linki</label>
                                      </div>
                                        ...
                                        <option value="0" selected>Açılır Menü Olsun </option>
                                        <option value="1" selected>Açılır Menü Olmasın </option>
                                        <?php
                                        $urun_kategori = $db->query("select * from menuler  order by id desc",PDO::FETCH_ASSOC);
