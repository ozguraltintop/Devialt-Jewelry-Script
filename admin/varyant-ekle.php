...
imagedestroy($content);
	unlink ('../resimler/'.$resim1);
	 ...
	 $simdi = $db->prepare("insert into varyant set fiyat=:fiyat,adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	$ekle = $simdi->execute(array("fiyat"=>$fiyat,"adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"eklenme_tarihi"=>$tarih));
	if($ekle){
		
		
		
		  $sonid=$db->query("select * from varyant order by id desc")->fetch(PDO::FETCH_ASSOC);
...
imagedestroy($image);
$content = imagecreatefromstring($cont);
$output = '../resimler/'.$seo.'-'.$randomm.'.webp';
$img1=$seo.'-'.$randomm.'.webp';
imagewebp($content,$output);
imagedestroy($content);
			unlink ('../resimler/'.$img);
    		$islem = $db->prepare("INSERT INTO urun_img SET urun_id = ?, img = ?,tur=?");
        	$islem = $islem->execute(array($yeni,$img1,$tur));
    	}}
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmpd))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM varyant WHERE id = '$duzenlenecek_id'")->fetch(PDO::FETCH_ASSOC);
	...
	else
	{
		...
		if ($_FILES["resim"]["type"] =="image/gif" || $_FILES["resim"]["type"] =="image/png"|| $_FILES["resim"]["type"] =="image/jpg"|| $_FILES["resim"]["type"] =="image/jpeg") 
		{
		    	...
		    	$ayar_kaydi = $db->query("SELECT * FROM varyant WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
...
imagedestroy($content);
	unlink ('../resimler/'.$img);
    		$islem = $db->prepare("INSERT INTO urun_img SET urun_id = ?, img = ?,tur=?");
        	$islem = $islem->execute(array($id,$img1,$tur));
    	}
    }
	 ...
	 $simdi1 = $db->prepare("update varyant set fiyat=:fiyat,adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	$ekle1 = $simdi1->execute(array("fiyat"=>$fiyat,"adi"=>$adi,"sira"=>$sira,"resim"=>$resim,"kategori"=>$kategori,"aciklama"=>$aciklama,"seo"=>$seo,"tur"=>$tur,"onaciklama"=>$onaciklama,"durum"=>$durum,"guncelleme_tarihi"=>$tarih,"id"=>$id));
	if($ekle1){
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from varyant where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="form-floating mb-3">
                                        ...
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Varyant Resmi</label>
