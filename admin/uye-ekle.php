...
$klasord="resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into uyeler set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from uyeler order by id desc")->fetch(PDO::FETCH_ASSOC);
...
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Ekibimiz Başarıyla Eklendi!</strong> 
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM uyeler WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update uyeler set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	...
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Ekibimiz Başarıyla Güncellendi!</strong>
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from uyeler where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Üye Ekle - <?=$ayar['site_title']?></title>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form method="post" enctype="multipart/form-data" >
                                        ...
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ürün Adı" value="<?=$guncelle['adsoyad']?>">
                                        <label for="floatingInput">Üye Adı</label>
                                      </div>
                                      
                                            <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ürün Adı" value="<?=$guncelle['email']?>">
                                        <label for="floatingInput">Üye Email</label>
                                      </div>
                                      
                                      <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ürün Adı" value="<?=$guncelle['telefon']?>">
                                        <label for="floatingInput">Üye Telefon</label>
                                      </div>
                                      
                                      <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ürün Adı" value="<?=$guncelle['normalsifre']?>">
                                        <label for="floatingInput">Üye Şifresi</label>
                                      </div>
                                      
                                       <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Ürün Adı" value="<?=$guncelle['tarih']?>">
                                        <label for="floatingInput">Üyelik Tarihi</label>
                                      </div>
