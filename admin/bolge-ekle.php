...
$klasord="../resimler/";
	...
	else
	{
	 ...
	 $simdi = $db->prepare("insert into hizmetbolgeleri set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,eklenme_tarihi=:eklenme_tarihi");
	...
	if($ekle){
		  ...
		  $sonid=$db->query("select * from hizmetbolgeleri order by id desc")->fetch(PDO::FETCH_ASSOC);
...
$yeni =$sonid['id'];
    if(isset($_POST['img'])){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Hizmet Bölgesi Başarıyla Eklendi!</strong>
...
if($_POST['kaydet'] and $_GET['islem']=='duzenle'){
	...
	if(empty($resim_tmp))
	{
		...
		$ayar_kaydi = $db->query("SELECT * FROM hizmetbolgeleri WHERE id = '$id'")->fetch(PDO::FETCH_ASSOC);
	...
	if(isset($_POST['img'])){
	 ...
	 $simdi1 = $db->prepare("update hizmetbolgeleri set adi=:adi,sira=:sira,resim=:resim,kategori=:kategori,durum=:durum,onaciklama=:onaciklama,aciklama=:aciklama,seo=:seo,tur=:tur,guncelleme_tarihi=:guncelleme_tarihi where id=:id");
	...
	if($ekle1){
		...
		<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                          <strong>Hizmet Bölgesi Başarıyla Güncellendi!</strong> 
                          <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
...
if($_GET['islem']=='duzenle'){
	...
	$guncelle = $db->query("select * from hizmetbolgeleri where id='$gid'")->fetch(PDO::FETCH_ASSOC);
}
...
<head>
        ...
        <title>Hizmet Bölgesi Ekle - <?=$ayar['site_title']?></title>
                ...
                <div class="main-wrapper">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="card-title">Hizmet Bölgesi  Ekle</h5>
                                    <p class="card-description">Lütfen boş alan bırakmayın.</p>
                                    <?=$mesaj?>
                                    <form method="post" enctype="multipart/form-data" >
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="sira" placeholder="Hizmet Bölgesi Sırası" value="<?=$guncelle['sira']?>">
                                        <label for="floatingInput">Hizmet Bölgesi sırası</label>
                                      </div>
                                      
                                      
                                           <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="floatingInput" name="adi" placeholder="Hizmet Bölgesi Adı" value="<?=$guncelle['adi']?>">
                                        <label for="floatingInput">Hizmet Bölgesi Adı</label>
                                      </div>
                                        ...
                                        <div class="mb-3">
                                        <label for="formFile" class="form-label">Hizmet Bölgesi Resmi</label>
                             ...
                             <?php
							 $bolgecek=$db->query("select * from bolge_kategori ")->fetchAll(PDO::FETCH_ASSOC);
							 foreach($bolgecek as $bolge){
							 ?>         
<option value="<?=$bolge['id']?>" <?php if($bolge['id']==$guncelle['kategori']){ echo "selected"; }?>><?=$bolge['adi']?> </option>                      
							 <?php }?>
                                      </select>
                                      </div>
                             ...
                             <div class="col-md-4 " style="margin:auto;padding:auto;">
				                    <div class="uploaddis aktif" data-id="1" style="margin:0 auto;">
				        			  <div class="upload1">
				        				  <span class="metin" style="width: 100%;float: left;">Hizmet Bölgesi Resimi Yükle</span>
